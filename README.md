...
一个以“完整笔记”为检索单位的中文图文搜索项目。每篇笔记由一段文本和一组图片组成；无论用户输入文字、图片还是两者，Top-K 返回的都是互不重复的完整笔记，而不是拆散的“文字—单图”对。
...
MUGE 等数据中，一段文字可能对应多张图片。若把每张图都拆成一个独立样本，同一篇笔记会占据多个排名，而且用户只能看到其中一张图。本项目从数据层就保留完整关系：
...
因此，Top 10 始终表示 10 篇不同的笔记。某篇笔记的图片相关性取其全部图片中的最高分，但返回结果会包含这篇笔记的完整图片列表。
...
文本索引和图片索引仍使用 `IndexHNSWFlat`、内积、`M=32`、`efConstruction=200`、`efSearch=512`。两份索引的行数允许不同：前者等于笔记数，后者等于全局唯一图片数。HNSW 保持在 CPU，GPU 只用于 Chinese-CLIP 查询编码。

## 已验证的模型结果

下表来自项目既有服务器产物，是 LoRA 模型在五项验证任务上的固定结果快照。仓库没有包含对应权重和数据，因此它不是“克隆即可复现”的承诺。
...
`doc_id` 默认由 `source:split:text_id` 组成；缺少 `text_id` 时回退到 `item_id`。同一 `doc_id` 的文本必须完全一致，图片按输入顺序去重。同一图片可以属于多篇笔记，向量只保存一次，召回后会展开到全部所属笔记。
...
迁移工具默认在 `atol=1e-6` 下验证同一笔记的重复文本向量、同一路径的重复图片向量是否一致，并拒绝覆盖任何已有目标目录。只有已经诊断出旧 FP16 分批编码漂移时，才应显式传入更大的 `--atol`；迁移报告会记录非完全相同行数和实际最大误差。旧目录保持不变。新代码遇到旧 schema 会直接提示运行该工具，不提供 pair-level 兼容分支。
